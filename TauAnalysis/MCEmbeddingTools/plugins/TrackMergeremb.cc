        ...
        if( trSeedRef.isAvailable() ) {
          ...
          for( auto tseed = bElSeeds; tseed != eElSeeds; ++tseed, ++sedref_it) {
            ...
            if (trSeedRef->isEcalDriven() && elSeedRef->isEcalDriven()) {
              ...
              // std::cout << " trSeedRef " << trSeedRef->caloCluster()->printHitAndFraction(0) << std::endl;
              // std::cout << " eleSeedRef " << elSeedRef->caloCluster()->printHitAndFraction(0) << std::endl;
              // std::cout << " trSeedRef " << trSeedRef->caloCluster()->printHitAndFraction(1)<< std::endl;
              // std::cout << " eleSeedRef " << elSeedRef->caloCluster()->printHitAndFraction(1)<< std::endl;
              // std::cout << "Seed trSeedRef " << trSeedRef->caloCluster()->seed().rawId() << std::endl;
              // std::cout << "Seed eleSeedRef " << elSeedRef->caloCluster()->seed().rawId() << std::endl;
              if ( trSeedRef->caloCluster()->hitsAndFractions()[0].first() == elSeedRef->caloCluster()->hitsAndFractions()[0].first() && trSeedRef->caloCluster()->hitsAndFractions()[1].first() == elSeedRef->caloCluster()->hitsAndFractions()[1].first()) 
               {
                // std::cout << "Found matching Seeds!" << std::endl;
