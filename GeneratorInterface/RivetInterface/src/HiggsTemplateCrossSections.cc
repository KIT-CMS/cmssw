...
*/      
	...
	return error(cat,HTXS::VH_IDENTIFICATION,"Failed to identify associated V-boson!");
      ...
      outfile << "Higgs pT: "<< cat.higgs.pt()<<std::endl;
      outfile << "Higgs y: "<< cat.higgs.rapidity()<<std::endl;
    ...
    void analyze(const Event& event) override {
      ...
      // Stage 1_1 enum offsets for each production mode: GGF=14, VBF=11, WH= 6, QQ2ZH=6, GG2ZH=6, TTH=2, BBH=2, TH=2
      vector<int> offset({0,1,15,26,32,38,44,46,48,50});
      int off = offset[P];
      // Stage 1_1 fine enum offsets for each production mode: GGF=21, VBF=25, WH= 16, QQ2ZH=16, GG2ZH=16, TTH=2, BBH=2, TH=2
      vector<int> offset1_1({0,1,22,47,63,79,95,97,99,101});
